...
getDataJSON(); // Obtener los datos del archivo addresses.json
...
const paintAddress = (antenas) => {
  ...
  Object.values(antenas).forEach( address => {
    ...
    cloneTMP.querySelector('.status').textContent = status_address[address.status].name + ': ' + address.type;
...
searchIP.addEventListener('keyup', () => {
  if (searchIP.value === '') {
    ...
    const results = Object.values(antenas).filter( element => 
      ...
      || element.address.includes(searchIP.value)
    );
...
editPing.addEventListener('submit', e => {
  ...
  if (formData.get('address') == IPv4Changed) {
